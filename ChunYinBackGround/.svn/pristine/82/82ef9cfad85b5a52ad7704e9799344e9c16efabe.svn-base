...
public class SongService {
	...
	public int insertSong(Song song) {
		...
		List<Composer> composerList = composerMapper.selectByExampleWithBLOBs(composerExample);
		if(composerList.size()!= 0){//存在该作曲人则直接给作曲人id赋值
			Composer composer = composerList.get(0);
	...
	public int updateSongInfo(Song song) {
		...
		com.chunyin.bean.ComposerExample.Criteria criteria = composerExample.createCriteria();
		criteria.andComposerNameEqualTo(composerName);
		List<Composer> composerList = composerMapper.selectByExample(composerExample);
